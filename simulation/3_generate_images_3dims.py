...
events_unique = my_utils.get_unique_events(f'{root_dir}/simulation/output.nur') # Obtain only the unique events for each particle
...
for iEvent, event in enumerate(events_unique):
    ...
    hdf5_path = f'simulation/mini-album_{iEvent}_n{stations_num}_stations.hdf5' # Save an album wrt multiplicity

    print(f'Event detected at {stations_num} stations. Saving to corresponding album at {hdf5_path}')
        ...
        for iChannel, channel in enumerate(station.iter_channels()):
            ...
            for iBin, hilb_bin in enumerate(binned_hilb):
                if len(hilb_bin) == 0: # If no voltages for this bin, do nothing
                    ...
                    image[iChannel][iBin][iStation] = np.mean(hilb_bin)
                elif BIN_MODE == 'MAX': # If MEAN BIN MODE is selected, calculate the max value
                    image[iChannel][iBin][iStation] = np.max(hilb_bin)
                else:
