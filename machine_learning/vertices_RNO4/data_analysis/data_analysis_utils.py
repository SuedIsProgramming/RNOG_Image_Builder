...
class ModelEval:
    ...
    def plot_true_reco(self, custom_title: str = None):
            ...
            loss_arr = self.data_tensors.target_arr.numpy()
            ...
            fig, axes = plt.subplots(2, 3, figsize=(20, 14), gridspec_kw={'height_ratios': [3, 1]})
    ...
    def plot_progression_true_reco(self,range: List[int] = None, show_every_n_checkpoints: int =0):
        """Iterates through all checkpoints in the checkpoint_dir, loads them,
            re-evaluates, and plots performance._summary_

        Args:
            range (List[int], optional): Array slicing options. Defaults to None.
            show_every_n_checkpoints (int, optional): Show every n checkpoints. Defaults to 0.

        Raises:
            ValueError: Value error if no checkpoint specified
        """
        ...
        if show_every_n_checkpoints > 1:
            files = files[1::show_every_n_checkpoints]
